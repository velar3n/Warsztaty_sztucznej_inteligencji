...
def run_cleaning():
    ...
    query = """
    SELECT
        CAST(molregno AS INTEGER),
        CAST(mw_freebase AS REAL),
        CAST(alogp AS REAL),
        CAST(hba AS INTEGER),
        CAST(hbd AS INTEGER),
        CAST(psa AS REAL),
        CAST(rtb AS INTEGER),
        CAST(aromatic_rings AS INTEGER),
        CAST(qed_weighted AS REAL)
    FROM compound_properties
    """

    logger.info(f"Reading compound_properties table...")
    df = read_table_psql(
        spark=spark,
        query=query,
